...
os.environ.setdefault("SBCA_FEATURES", "data/bert_pred_for_SARL.csv")
import run_all_old as R          # pre-correction features, kept for the audit
...
def run_group(stocks, suffix):
    ...
    df_rets.to_csv(f"./runs_pre_correction/ablation_rets_{suffix}.csv", index=False)
    pd.DataFrame(metrics_rows).to_csv(f"./runs_pre_correction/ablation_metrics_{suffix}.csv", index=False)
    pd.DataFrame(pv_series).to_csv(f"./runs_pre_correction/ablation_pv_{suffix}.csv", index=False)
    return rets, metrics_rows


def main():
    os.makedirs("./runs_pre_correction", exist_ok=True)
    stock_groups = [
