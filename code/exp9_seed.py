...
KEY=[(L[:4],'4Large'),(L[:8],'8Large'),(M[:4],'4Mid'),(M[:8],'8Mid'),
...
if not os.path.exists(out):
    ...
    for stks,gn in KEY:
        ...
        for sd in SEEDS:
            ...
            pv=bt(m,env,TEST_);ev=evaluate(pv);srs.append(ev['SR'])
            print(f"{gn} seed={sd}: SR={ev['SR']:.4f}")
        mean_sr=np.mean(srs);std_sr=np.std(srs)
        rows.append([gn,mean_sr,std_sr,round(std_sr/abs(mean_sr)*100,2) if abs(mean_sr)>1e-4 else 0])
        print(f"  {gn}: SR={mean_sr:.4f}+-{std_sr:.4f}")
    pd.DataFrame(rows,columns=["Group","SR_mean","SR_std","CV%"]).to_csv(out,index=False)
print("Done.")
