...
if not os.path.exists(out):
    ...
    for stks,gn in KEY:
        ...
        for lr_val in LR_GRID:
            ...
            pv=bt(m,env,TEST_);ev=evaluate(pv)
            rows.append([gn,na,lr_val,ev['PV'],ev['SR'],ev['Sortino'],ev['MDD'],ev['AR'],ev['Calmar']])
            print(f"{gn} LR={lr_val}: SR={ev['SR']:.4f}")
    pd.DataFrame(rows,columns=["Group","N","LR","PV","SR","Sortino","MDD","AR","Calmar"]).to_csv(out,index=False)
print("Done.")
