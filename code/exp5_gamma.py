"""
exp5_gamma.py — Discount Factor Grid Search
==================================================================
Gamma in {0.9, 0.93, 0.95, 0.97, 0.99} across 6 groups.
Fixed: SBCA, EA=0.4, W=30, COMM=0.0025, TP=0.005
Output: result/exp5_gamma.csv
"""
...
W,COMM,LRISK,EA,TP=30,0.0025,0.1,0.4,0.005;EP,PAT,LR,ENT=30,5,3e-4,0.1
GAMMA_GRID=[0.9,0.93,0.95,0.97,0.99]
...
def train_ac(model,env,tr,vr,gam,desc=""):
...
out=f"{BASE}/result/exp5_gamma.csv"
if not os.path.exists(out):
    ...
    for stks,gn in KEY:
        ...
        for gam in GAMMA_GRID:
            ...
            m=train_ac(m,env,TR,VR,gam,f"GAM{gam}")
            pv=bt(m,env,TEST_);ev=evaluate(pv)
            rows.append([gn,na,gam,ev['PV'],ev['SR'],ev['Sortino'],ev['MDD'],ev['AR'],ev['Calmar']])
            print(f"{gn} gamma={gam}: SR={ev['SR']:.4f}")
    pd.DataFrame(rows,columns=["Group","N","Gamma","PV","SR","Sortino","MDD","AR","Calmar"]).to_csv(out,index=False)
print("Done.")
