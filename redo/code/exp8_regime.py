...
if not os.path.exists(out):
    ...
    for stks,gn in KEY:
        ...
        for rn,(rs,re) in REGIMES.items():
            ...
            pv_s,ato_s=bt_cm(ms,env_rl,te_r);ev_s=evaluate(pv_s)
            pv_p,ato_p=bt_pp(mp,env_rl,te_r);ev_p=evaluate(pv_p)
            pv_e=ew(env_bm,te_r);ev_e=evaluate(pv_e)
            pv_b=bh(env_bm,te_r);ev_b=evaluate(pv_b)
            for name,ev,ato in [('SBCA',ev_s,ato_s),('PPO',ev_p,ato_p),('EW',ev_e,0),('BH',ev_b,0)]:
                rows.append([gn,rn,name,ev['PV'],ev['AR'],ev['SR'],ev['Sortino'],ev['MDD'],ev['Calmar'],round(ato,4)])
            print(f"{gn} {rn}: SBCA={ev_s['SR']:.4f} PPO={ev_p['SR']:.4f} EW={ev_e['SR']:.4f} BH={ev_b['SR']:.4f}")
    pd.DataFrame(rows,columns=["Group","Regime","Model","PV","AR","SR","Sortino","MDD","Calmar","ATO"]).to_csv(out,index=False)
print("Done.")
