"""
exp9_seed.py — Multi-Seed Stability Test
==================================================================
5 random seeds across 11 groups.
Output: result/exp9_seed.csv (SR mean +/- std)
"""
...
KEY=[(L_sorted[:4],"4Large"),(L_sorted[:8],"8Large"),(M_sorted[:4],"4Mid"),(M_sorted[:8],"8Mid"),
...
out=f"{BASE}/result/exp9_seed.csv"
if not os.path.exists(out):
    ...
    for stks,gn in KEY:
        ...
        env=Env(ca,da);evs=[]
        for sd in SEEDS:
            set_seed(sd);m=SBCA(W,na).to(DEVICE)
            m=train_ac(m,env,TR,VR,f"SBCA_s{sd}")
            pv,ato=bt(m,env,TEST_);ev=evaluate(pv);ev['ATO']=ato;evs.append(ev)
            print(f"{gn} seed={sd}: SR={ev['SR']:.4f} So={ev['Sortino']:.4f} ATO={ato:.4f}")
        cols=['PV','SR','Sortino','MDD','AR','Calmar','ATO']
        means={c:np.mean([e[c] for e in evs]) for c in cols}
        stds={c:np.std([e[c] for e in evs]) for c in cols}
        row=[gn]+[round(means[c],4) for c in cols]+[round(stds[c],4) for c in cols]+[round(stds['SR']/abs(means['SR'])*100,2) if abs(means['SR'])>1e-4 else 0]
        rows.append(row)
        print(f"  {gn}: PV={means['PV']:.4f} SR={means['SR']:.4f}+-{stds['SR']:.4f} ATO={means['ATO']:.4f}")
    pd.DataFrame(rows,columns=["Group","PV_mean","SR_mean","Sortino_mean","MDD_mean","AR_mean","Calmar_mean","ATO_mean","PV_std","SR_std","Sortino_std","MDD_std","AR_std","Calmar_std","ATO_std","CV%"]).to_csv(out,index=False)
print("Done.")
