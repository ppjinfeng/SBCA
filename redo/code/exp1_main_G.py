"""
exp1_main_G.py — Main experiment: SBCA vs. baselines, 24 stocks, FinBERT
Reads: data/bert_pred_24stocks_delta_FinBERT.csv
"""
...
# New 24 stocks (S&P tiers)
LARGE = 'GILD XOM CVX MRK WFC ORCL DIS CAT'.split()
MID   = 'WDC BIIB KEY CLX RRC DECK CI FTI'.split()
SMALL = 'KSS ANF URBN DHI REGN CF LEN DDS'.split()

# ========== Load FinBERT data ==========
print("Loading FinBERT scores...")
delta_csv = f"{BASE}/data/bert_pred_24stocks_delta_FinBERT.csv"
df = pd.read_csv(delta_csv); df['Date'] = pd.to_datetime(df['Date'])
print(f"FinBERT delta: mean={df['delta_bert'].mean():.3f}, std={df['delta_bert'].std():.3f}")
...
class CM(nn.Module):
    def __init__(s,pd,td,h=128):
        ...
        s.te=nn.Sequential(nn.Linear(td,h),nn.LayerNorm(h),nn.ReLU()); s.gamma=nn.Linear(h,h); s.beta=nn.Linear(h,h); s.fu=nn.Sequential(nn.Linear(h,h),nn.ReLU())
...
class SBP(nn.Module):
    def __init__(s,sd,n): super().__init__(); s.na=n; s.net=nn.Sequential(nn.Linear(sd,256),nn.LayerNorm(256),nn.ReLU(),nn.Linear(256,128),nn.ReLU(),nn.Linear(128,n),nn.Softmax(-1))
