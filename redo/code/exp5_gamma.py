...
if not os.path.exists(out):
    ...
    for stks,gn in KEY:
        ...
        for gam in GAMMA_GRID:
            ...
            rows.append([gn,na,gam,ev['PV'],ev['SR'],ev['Sortino'],ev['MDD'],ev['AR'],ev['Calmar'],round(ato,4)])
            print(f"{gn} gamma={gam}: SR={ev['SR']:.4f}")
    pd.DataFrame(rows,columns=["Group","N","Gamma","PV","SR","Sortino","MDD","AR","Calmar","ATO"]).to_csv(out,index=False)
print("Done.")
