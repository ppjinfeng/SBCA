"""
exp6_lr.py — Learning Rate Grid Search
==================================================================
LR in {1e-4, 2e-4, 3e-4, 5e-4, 1e-3} across 4 groups.
Fixed: SBCA, GAM=0.99, EA=0.4, TP=0.005, W=30, COMM=0.0025
Groups: 4Large/8Large/4Small/8Small
Output: result/exp6_lr.csv
"""
...
W,COMM,GAM=30,0.0025,0.99;LRISK,EA,TP=0.1,0.4,0.005;EP,PAT,ENT=30,5,0.1
LR_GRID=[1e-4,2e-4,3e-4,5e-4,1e-3]
...
KEY=[(L_sorted[:4],"4Large"),(L_sorted[:8],"8Large"),(S_sorted[:4],"4Small"),(S_sorted[:8],"8Small")]
...
def train_ac(model,env,tr,vr,lr_val,desc=""):
    opt=optim.AdamW(model.parameters(),lr=lr_val,weight_decay=1e-5);sch=optim.lr_scheduler.ReduceLROnPlateau(opt,'max',0.5,1)
...
if not os.path.exists(out):
    ...
    for stks,gn in KEY:
        ...
        for lr_val in LR_GRID:
            ...
            m=train_ac(m,env,TR,VR,lr_val,f"LR{lr_val}")
            pv,ato=bt(m,env,TEST_);ev=evaluate(pv)
            rows.append([gn,na,lr_val,ev['PV'],ev['SR'],ev['Sortino'],ev['MDD'],ev['AR'],ev['Calmar'],round(ato,4)])
            print(f"{gn} LR={lr_val}: SR={ev['SR']:.4f}")
    pd.DataFrame(rows,columns=["Group","N","LR","PV","SR","Sortino","MDD","AR","Calmar","ATO"]).to_csv(out,index=False)
print("Done.")
